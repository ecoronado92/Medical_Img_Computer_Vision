...
test_df <- bind_cols(y = y_test, X_test)
samp_df <- test_df %>% 
  group_by(y) %>% 
...
X_test_reshape  <- array_reshape(matrix(test_df[, -1]),
                                 dim = c(nrow(test_df), 50, 50, 3))
X_train_reshape <- array_reshape(matrix(train_df[, -1]),
...
train_generator <- flow_images_from_data(x = X_train_reshape,#[1:tune,,,], 
                                         y = to_categorical(train_df[,1], num_classes),#[1:tune,],
                                         batch_size = batch_size,
                                         generator = datagen,
                                         seed = 2)
...
hist <- model %>%
  fit_generator(train_generator,
                ...
                validation_data = list(X_test_reshape,#[1:test_tune,,,],
                                       to_categorical(test_df[,1], num_classes)),#[1:test_tune,]),
                verbose = 1,
